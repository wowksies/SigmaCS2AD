...
// Hardcoded known IDs from SellAuth Developer dashboard — used as fallback when env vars are absent
const KNOWN_PRODUCT_ID = 634549;
const KNOWN_SHOP_URL = 'https://omniscs2.mysellauth.com';

const PLAN_CONFIG = Object.freeze({
  three_days: {
    label: '4 Days',
    envKey: 'SELLAUTH_VARIANT_3_DAYS_ID',
    knownVariantId: 995693,
...
function getConfiguredProductId() {
  return parseOptionalInt(process.env.SELLAUTH_PRODUCT_ID) || KNOWN_PRODUCT_ID;
}

function getConfiguredStorefrontUrl() {
  const value = String(process.env.SELLAUTH_STOREFRONT_URL || '').trim();
  return value ? value.replace(/\/+$/, '') : KNOWN_SHOP_URL;
}
...
function getVariantId(planKey) {
  const config = PLAN_CONFIG[planKey];
  if (!config) return null;
  return parseOptionalInt(process.env[config.envKey]) || config.knownVariantId || null;
}
...
function resolveVariantIdFromProduct(product, planKey) {
  const envOverride = getVariantId(planKey);
  if (envOverride) {
    return envOverride;
  }
...
async function resolveCheckoutCatalog(options = {}) {
  ...
  // Build variants map from env vars + hardcoded known IDs (always resolves)
  const resolvedVariants = Object.fromEntries(
    Object.keys(PLAN_CONFIG).map((planKey) => [planKey, getVariantId(planKey)])
  );
  const hasAllIds = Boolean(
    configuredProductId &&
    configuredStorefrontUrl &&
    Object.values(resolvedVariants).every(Boolean)
  );
  ...
  // Fallback: query the SellAuth API to discover product/variant IDs dynamically
  if (!shopId) {
  ...
  // If the product endpoint didn't include variant IDs, try the dedicated variants endpoint
  ...
  if (!anyResolved && productId && shopId) {
    ...
    if (variantList.length > 0) {
      Object.keys(PLAN_CONFIG).forEach((planKey) => {
        variants[planKey] = getVariantId(planKey) || resolveVariantIdFromList(variantList, planKey);
      });
    }
  }
...
function buildCheckoutLink(catalog, planKey) {
  ...
  // SellAuth requires literal brackets in the query string (PHP-style arrays).
  // URLSearchParams would encode them as %5B%5D which SellAuth rejects.
  const base = String(catalog.shopUrl).replace(/\/+$/, '');
  return `${base}/checkout-link?cart[0][productId]=${catalog.productId}&cart[0][variantId]=${variantId}&cart[0][quantity]=1`;
}
